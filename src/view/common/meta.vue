<template>
    <div>
        <meta http-equiv="X-UA-Compatible" content="IE=edge" />
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <meta content="text/html; charset=UTF-8; X-Content-Type-Options=nosniff" http-equiv="Content-Type" />
        <!-- 2017.11.24 SMP 취약점
        "Content-Security-Policy" 헤더가 누락되었습니다. 
        "X-Content-Type-Options" 헤더가 누락되었습니다. 
        "X-XSS-Protection" 헤더가 누락되었습니다. -->
        <meta http-equiv="Content-Security-Policy" content="object-src 'self'">
        <meta http-equiv="X-XSS-Protection" content="'1; mode=block' always">
    
        <meta http-Equiv="Cache-Control" Content="no-cache" />
        <meta http-Equiv="Pragma" Content="no-cache" />
        <meta http-Equiv="Expires" Content="0" />
        <title>kt그룹 국가정보통신서비스</title>
        <link rel="shortcut icon" type="image/x-icon" href="/favicon.ico" >
        <link href="/icon.png" rel="apple-touch-icon" />
        <link href="/icon.png" media="screen and (resolution: 132dpi)" rel="apple-touch-icon" />
        <link href="/icon.png" media="screen and (resolution: 163dpi)" rel="apple-touch-icon" />
        <link href="/icon.png" media="screen and (resolution: 326dpi)" rel="apple-touch-icon" />
    </div>
</template>

<script>
export default {
    name: 'Meta',
    // Add any necessary Vue component options here
}
</script>

<style scoped>
/* Add any necessary styles here */
</style>